...
/******************************************************************************/
TreeObject::TreeObject(std::string name, BlkNumType blknum, PartitionManager* pm)
:_name(name), _blockNumber(blknum), _myPartitionManager(pm)
{}

TreeObject::~TreeObject(){}

std::string TreeObject::getName(){return _name;}

void TreeObject::setName(std::string name){_name = name;}
...
unordered_map<std::string, TreeObject*>::iterator TreeObject::begin(){return _myTree.begin();}

unordered_map<std::string, TreeObject*>::iterator TreeObject::end(){return _myTree.end();}

void TreeObject::insert(std::string name, TreeObject* ptr)
{
  auto ret = _myTree.insert(pair<std::string, TreeObject*>(name, ptr));
  if(!ret.second)
  {
...
void TreeObject::erase(std::string name)
{
...
TreeObject* TreeObject::find(std::string name)
{
...
void RootTree::readIn(std::unordered_multimap<std::string, FileInfo*>* allFiles, RootTree* rootTree)
{
  char* buff = new char[_myPartitionManager->getBlockSize()];
  memset(buff, 0, _myPartitionManager->getBlockSize()); //zero out memory
  Index currentIndex{_blockNumber, 0};
...
//   _name.assign(buff + currentIndex.offset, 5);
  ...
  while(currentIndex != EOFIndex)
  {
    if(buff[currentIndex.offset] != 0)
    {
      ...
      auto it_ret = _myTree.insert(pair<std::string, TagTree*>(tagName, tagTree));
      if(!it_ret.second)
      {
...
TagTree::TagTree(std::string tagName, BlkNumType blknum, PartitionManager* pm)
...
void TagTree::readIn(std::unordered_multimap<std::string, FileInfo*>* allFiles, RootTree* rootTree)
{
  ...
  while(currentIndex != EOFIndex)
  {
    if(buff[currentIndex.offset] != 0)
    {
      ...
      auto it_ret = _myTree.insert(pair<std::string, FileInfo*>(finode->mangle(), finode));
//       cout << "File just inserted(maybe): " << finode->mangle() << endl;
      ...
      if(!found) 
      {
        allFiles->insert(pair<std::string, FileInfo*>(fileName, finode));
      }
...
/******************************************************************************/
FileInfo::FileInfo(std::string fileName,BlkNumType blknum, PartitionManager* pm)
...
void FileInfo::readIn(std::unordered_multimap<std::string, FileInfo*>* allFiles, RootTree* rootTree)
{
  ...
  if(contBlock != 0)
  {
    ...
    for(int i = 0; i < localTagCount; i++)
    {
      ...
      _myTree.insert(pair<std::string, TreeObject*>(tagName, rootTree->find(tagName)));
    }
...
void FileInfo::insert(std::string name, TreeObject* ptr)
{
  /*Check for max tags*/
...
std::string FileInfo::mangle(std::vector<std::string>& tags)
{
  map<std::string, int> tempTags;
  std::string ret = _name;
  
  for(size_t i = 0; i < tags.size(); i++)
  {
    tempTags.insert(pair<std::string, int>(tags[i], 0));
  }
  
  for(auto it = _myTree.begin(); it != _myTree.end(); it++)
  {
    tempTags.insert(pair<std::string, int>(it->first, 0));
  }
