...
PartitionManager::PartitionManager(DiskManager *dm, std::string partitionName)
{
  myDM = dm;
  ...
  if(blknum == 0)
  {
    ...
    std::cerr << "Partition" << _partitionName << " has run out of space!" << std::endl;
...
BlkNumType PartitionManager::getFreeDiskBlock()
{
  ...
  if(blknum == 0)
  {
    ...
    std::cerr << "WARNING! Last free block allocated" << std::endl;
  }
...
std::string PartitionManager::getPartitionName()
{
