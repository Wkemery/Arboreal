...
2. **Using the following syntax,  edit the file as you see fit:**
	...
	* **There are restrictions on values for** `diskInfo`**:**  
		...
		* There exists a **hardcap on the number of tags that can be associated with a file** and it is equal to `( ( ( block size - filename size - 136 ) / sizeof( BlkNumType ) ) + ( block size / sizeof( BlkNumType ) )`
